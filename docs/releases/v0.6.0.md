...
release-agent-team install kiro

# Apply the installation
release-agent-team install kiro --apply

# Use a custom prefix for multi-project installations
release-agent-team install kiro --prefix myproject --apply
```

Installed files are prefixed with the team name (default: `release-agent-team`) to avoid collisions:

```
~/.kiro/agents/release-agent-team_pm.json
~/.kiro/agents/release-agent-team_qa.json
